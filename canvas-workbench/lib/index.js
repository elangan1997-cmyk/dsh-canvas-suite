        ...
        if (pathname === '/dsh-canvas/export-text-psd' && req.method === 'POST') {
          ...
          try {
            ...
            const jsx = '#target photoshop\n(function(){\n'
              ...
              + 'try{app.preferences.rulerUnits=Units.PIXELS; try{app.preferences.typeUnits=TypeUnits.PIXELS;}catch(typeUnitErr){} var doc=app.open(new File(cfg.input)); var docW=Number(doc.width.as("px")),docH=Number(doc.height.as("px")); var list=cfg.blocks||[]; for(var i=0;i<list.length;i++){var b=list[i]||{}; var text=String(b.text||"").replace(/^[\\s\\r\\n]+|[\\s\\r\\n]+$/g,""); if(b.enabled===false||!text)continue; var boxH=Math.max(12,Number(b.height||0)); var fontSize=Math.max(10,Math.min(docH*.5,Number(b.fontSize||0)>0?Number(b.fontSize):boxH*.82)); var targetX=Math.max(0,Math.min(docW-2,Number(b.x||0))); var targetY=Math.max(0,Math.min(docH-2,Number(b.y||0))); var layer=doc.artLayers.add(); layer.kind=LayerKind.TEXT; layer.name="文字 "+(i+1)+" · "+text.substr(0,24); var ti=layer.textItem; ti.contents=text; ti.position=[UnitValue(0,"px"),UnitValue(fontSize,"px")]; try{ti.size=UnitValue(fontSize,"px");}catch(sizeErr){ti.size=fontSize;} try{ti.font=String(b.fontPostScript||b.fontFamily||"MicrosoftYaHei");}catch(fontErr){try{ti.font="MicrosoftYaHei";}catch(fontFallbackErr){try{ti.font="ArialMT";}catch(e){}}} ti.color=rgb(b.color); try{ti.tracking=Math.max(-200,Math.min(1000,Number(b.letterSpacing||0)));}catch(trackErr){} try{if(Number(b.lineHeight||0)>0)ti.leading=UnitValue(Math.max(fontSize,Number(b.lineHeight)),"px");}catch(leadErr){} try{ti.justification=Justification.LEFT;}catch(justErr){} layer.visible=true; try{var bounds=layer.bounds; var actualH=Math.max(1,Number(bounds[3].as("px"))-Number(bounds[1].as("px"))); var targetH=Math.max(10,Math.min(docH-targetY,boxH)); var scale=Math.max(10,Math.min(1000,targetH*100/actualH)); layer.resize(scale,scale,AnchorPosition.TOPLEFT); bounds=layer.bounds; var left=Number(bounds[0].as("px")),top=Number(bounds[1].as("px")); layer.translate(UnitValue(targetX-left,"px"),UnitValue(targetY-top,"px"));}catch(placeErr){} try{ti.justification=String(b.textAlign||"left")==="center"?Justification.CENTER:String(b.textAlign||"left")==="right"?Justification.RIGHT:Justification.LEFT;}catch(justErr2){} try{if(Math.abs(Number(b.rotation||0))>0.01)layer.rotate(Number(b.rotation));}catch(rotateErr){}} for(var g=0;g<doc.layerSets.length;g++){try{if(String(doc.layerSets[g].name)==="OCR text preview - replace in Photoshop")doc.layerSets[g].visible=false;}catch(groupErr){}} var opts=new PhotoshopSaveOptions(); opts.layers=true; doc.saveAs(new File(cfg.output),opts,true,Extension.LOWERCASE); doc.close(SaveOptions.DONOTSAVECHANGES); restoreUnits(); }catch(err){restoreUnits(); try{if(doc)doc.close(SaveOptions.DONOTSAVECHANGES);}catch(closeErr){} throw err;}\n})();\n';
