...
- **安装**：远程驱动（上面两条）不需要任何安装——DSH 启动时自动准备脚本副本。想在 PS/AI 的「文件 → 脚本」菜单里也有面板，点画布「更多 → 🔐 安装 PS / AI 菜单面板」，输入一次 Mac 管理员密码（系统对话框，插件接触不到密码），重启 PS/AI 后菜单里出现「DSH画布桥接-…」。两款应用都只扫描自己应用目录里的 Scripts（root 权限），用户目录放脚本无效。
- **备用入口（Adobe 侧发起）**：PS/AI 重启后「文件 → 脚本 → DSH画布桥接-…」打开面板：发送选中图层 / 发送整个文档 (PSD / .ai 副本) / 置入 / 打开为新文档。面板是模态对话框（两款应用的脚本环境都不支持常驻面板），用完点「关闭」。
