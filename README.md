...
在 PS / AI 里装一个脚本面板，图层可以送到画布编辑，编辑完一键回到原位：
- **发送**：PS 面板「发送选中图层 → 画布」（逐层或合并为一张，透明 PNG，裁到图层边界）或「发送整个文档 (PSD)」；AI 面板「发送选中对象」（72/150/300 dpi）或「发送整个画板 (.ai 副本)」。文件落在项目 `ADOBE桥接/来自Photoshop|Illustrator/`，画布约 3 秒内自动长出来。
- **返回**：画布选中图片，工具栏「→Ps」/「→Ai」把它（图片按图片、分层 PSD/.ai 按分层，原样）放进 `ADOBE桥接/发件箱/`；回到 PS/AI 打开面板（打开时自动检测，之后点「刷新」），「置入为图层」会缩放并平移**回到出发位置**（文档名一致时；也可勾选总是归位），或「打开为新文档」直接编辑分层。真机验收：PS 2025 与 AI 2026 往返均像素级归位。
- 面板是模态对话框（Photoshop 与 Illustrator 的脚本环境都不支持常驻面板）：用完点「关闭」，下次从菜单再开。
- **安装**：画布「更多 → 🔗 安装 Adobe 桥接脚本」，重启 PS/AI 后在「文件 → 脚本」里打开「DSH画布桥接」。macOS 下 Photoshop 装进用户级目录不需要管理员；Illustrator 的目录属于 root，界面会给出一条可粘贴的 `sudo` 命令，或直接用「文件 → 脚本 → 其它脚本…」打开 `~/.dsh/canvas-workbench/adobe-bridge/scripts/` 里的脚本。
- 全程只靠文件夹传输和一个握手文件，没有端口、网络权限或签名要求，CS6 到 2026 通用。协议与排障速查见 [`canvas-workbench/adobe-bridge/PROTOCOL.md`](canvas-workbench/adobe-bridge/PROTOCOL.md)。
