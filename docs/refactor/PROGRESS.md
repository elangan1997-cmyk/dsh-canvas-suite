...
- [2026-09-17·SVG 图层编辑] svg_layers.py（list / replace-image）+ 路由 svg 分支 + iframe 放行 .svg。E2E：列出（背景位图 + 2 文字对象）→ Codex 改淡紫 → 内嵌位图替换，文字无损、尺寸保持 800×1000。Python 工具 12→13。
- [2026-09-17·可视化图层编辑] /document-layers 附缩略图（psd --thumbs / AI 低清 imageCapture≤12 项 / SVG 解码）；新增 /extract-layer（提取为临时画布图）；/edit-layer 支持 maskData（prepare_mask + composite_edit 擦除管线）；iframe add-image 支持 customData+openEditor（经 openImageEditorById 全局钩子进入现有编辑器）；父层 request-image-edit 分流 layerEdit → edit-layer，成功后 remove-sources 临时元素+占位并加入新版本。E2E：缩略图 ✓、提取 600×800 ✓、蒙版擦除 27s ✓（文字层无损）。
- [2026-09-17·AI 两文件问题] 实测证据链：AI2026 `typeof CloseOptions==='undefined'`（close 两种写法均抛错）→ 生成/图层编辑的临时文档全部遗留（复测时发现 3 个）；脚本成功后跳过 open。修复：runIllustratorJsx/runPhotoshopJsx finally 里 AppleScript close every document saving no；text.routes 生成后总是 open 画布正式文件。E2E：生成→AI 里仅打开正式文件（同名同路径）→AppleScript 关闭后无遗留。另确认画布 mtime 轮询本身正常（此前 touch 未刷新是因为当前聊天绑定的画布项目是另一个目录）。
