...
- **文字重建新增 SVG（Illustrator）导出**：识别确认后面板提供「生成 SVG（Illustrator）」，与 PSD 共用同一套框选/文字/背景清理逻辑；产物为内嵌背景位图 + 可编辑 `<text>` 的 SVG（自动加入画布并尝试用 Illustrator 打开）。字体映射为家族名 + 字重/样式（普惠体/思源/Inter 等，本机安装即可在 AI 中直接编辑）；背景清理成功时文字组可见，未清理时文字组写入但隐藏（避免与原图文字重叠，图层面板可开启）。
