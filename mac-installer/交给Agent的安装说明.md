...
> 使用方式：把本说明和 GitHub Release 中的 `DSH-Canvas-Suite-1.5.9-macOS-Complete.dmg` 一起交给本机 Agent，并告诉它：“请严格按说明完成安装、配置和验证。”
...
- `DSH-Canvas-Suite-1.5.9-macOS-Complete.dmg`
...
```bash
cd "/安装包所在目录"
shasum -a 256 -c "DSH-Canvas-Suite-1.5.9-macOS-Complete.dmg.sha256"
```
...
```bash
DMG_PATH="/实际路径/DSH-Canvas-Suite-1.5.9-macOS-Complete.dmg"
